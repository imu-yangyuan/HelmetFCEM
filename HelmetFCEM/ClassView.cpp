...
{
	hRoot = m_wndClassView.InsertItem(_T("指标树"), 0, 0);
...
void CClassView::showTree(CHelmetFCEMDoc *pDoc)
{
	/*HTREEITEM hRoot = m_wndClassView.InsertItem(_T("FakeApp 类"), 0, 0);
	m_wndClassView.SetItemState(hRoot, TVIS_BOLD, TVIS_BOLD);*/
	if(!settree)
	{
		HTREEITEM fisrtLayer,Htemp;
		for (int i=0;i<pDoc->quotaNum;i++)
		{
			if(pDoc->finalQuota[i].treeRank==1&&pDoc->finalQuota[i].childNum!=0){
				fisrtLayer=m_wndClassView.InsertItem(pDoc->quotaName[i], 1, 1, hRoot);
				for (int j=0;j<pDoc->quotaNum;j++)
				{
					if(pDoc->finalQuota[i].id==pDoc->finalQuota[j].parentId&&pDoc->finalQuota[j].parentId!=-1){
						Htemp=m_wndClassView.InsertItem(pDoc->quotaName[j], 3, 3, fisrtLayer);
						m_wndClassView.Expand(Htemp, TVE_EXPAND);
					}
				}
			}else if(pDoc->finalQuota[i].treeRank==1&&pDoc->finalQuota[i].childNum==0){
				Htemp=m_wndClassView.InsertItem(pDoc->quotaName[i], 1, 1, hRoot);
				m_wndClassView.Expand(Htemp, TVE_EXPAND);
			}
		}
		m_wndClassView.Expand(hRoot, TVE_EXPAND);
